...
import { Constants } from "./init/init.js"
import getRedisStatusChannel from "./utils/getRedisChannel.js"
import { config } from "./config/index.js"
/**
 * Redis Variables ::
 */

const constants = await Constants.init()

await Constants.setup()
let redis = Constants.redis,
  channel = Constants.channel,
  subscriber = Constants.subscriber,
  pushBrowser = Constants.pushBrowser

subscriber.on(`message`, (domainInfo, message) => {
  if (domainInfo != getRedisStatusChannel()) {
  ...
  // call domain
  const { domain, linkQueue, authentication, maxPages, limit } =
    JSON.parse(message)
  startConsumers(domain, linkQueue, authentication, maxPages, limit).catch(
    (err) => {
      console.error(
...
async function startConsumers(
  ...
  console.log(
    `[${ID}] :: Starting Consumer for Domain :: ${domain}, Queue :: ${linkQueue}, MaxPages :: ${maxPages}, Limit :: ${limit}\n\n`,
  )
  ...
  try {
    ...
    console.log(
      `[${ID}] :: Error Ocurred, Queue does not exist in RabbitMQ server\nQueue :: ` +
        linkQueue,
    )
  ...
  async function cleanup(consumerTag) {
    ...
    console.log(
      `[${ID}] :: Completed Scraping for Domain :: ${domain} in ${completionTime} seconds`,
    )
  ...
  async function setPauseBrowser(consumerTag) {
    ...
    pauseStatusTimeout = setTimeout(() => {
      checkPauseStatus(consumerTag)
    }, 10000)
  }
  ...
  const { consumerTag } = await channel.consume(linkQueue, async (msg) => {
    try {
      ...
      if (data.link === undefined || data.depth === undefined) {
        console.error(
          `[${ID}] :: ERROR :: Invalid type of data found in LinkChannel in Puppeteer`,
        )
      ...
      console.log(
        `[${ID}] :: Processing Link :: ${data.link} at Depth :: ${data.depth}`,
      )
